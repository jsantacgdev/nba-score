...
as $$
  ...
  relevantes as (
    select u.s as season from unnest(target_seasons) u(s)
    union
    select (select max(t.season) from todas t where t.season < u.s)
    from unnest(target_seasons) u(s)
  ),
  base as (
    select
    ...
    where g.status = 'final'
      ...
      and g.season in (select r.season from relevantes r where r.season is not null)
  ),
  lineas as (
    select
  ...
  anterior as (
    select
      t.season,
      (select max(t2.season) from todas t2 where t2.season < t.season) as previa
    from todas t
  ),
  net_temporada as (
